...
### Branching with `messageTree`

If your backend stores message history as a tree (e.g., each message has a `parentId`), use `messageTree` instead of `messages` to enable branch switching in the UI:
...
const runtime = useExternalStoreRuntime({
  messageTree: {
    ...
    headId: activeLeafId, // controls which branch is displayed
  },
  convertMessage,
  setMessages, // required for branch switching UI
  onNew,
});
```

<Callout type="info">
  `messageTree` reuses `convertMessage` so you don't need to manually construct `ThreadMessage` objects. Unlike the deprecated `messageRepository` prop, you pass your own message type and let the runtime handle conversion.
</Callout>
  ...
  parameters={[
    {
      name: "messages",
      type: "readonly T[]",
      description: "Linear array of messages from your state. Provide either `messages` or `messageTree`.",
      required: true,
    },
    {
      name: "messageTree",
      type: "MessageTree<T>",
      description: "Tree of messages with parent-child relationships for branching support. Provide either `messages` or `messageTree`. Use with `convertMessage` to convert your message format.",
    },
    {
      name: "onNew",
...
A tree structure for messages with branching support.
...
<ParametersTable
  ...
  parameters={[
    {
      name: "messages",
      type: "readonly MessageTreeItem<T>[]",
      description: "All messages in the tree, each with a parentId reference",
      required: true,
    },
    {
      name: "headId",
      type: "string | null",
      description: "ID of the leaf message for the active branch. Defaults to the last message in the array.",
    },
  ]}
/>
...
A single item in a message tree.

<ParametersTable
  type="MessageTreeItem<T>"
  parameters={[
    {
      name: "message",
      type: "T",
    ...
    },
    {
      name: "parentId",
      type: "string | null",
      description: "ID of the parent message, or null for root messages",
      required: true,
    },
  ]}
/>
